...
import { logout, profileData } from '@/src/store/userDataSlice';
import { signOut, useSession } from 'next-auth/react';
import { Menu, X } from 'lucide-react';
import { IoIosArrowDown } from "react-icons/io";
import { IoIosArrowUp } from "react-icons/io";
import ThemeToggle from './theme-toggle';
import NotificationDropdown from "@/src/components/sections/notifications/NotificationDropdown"
...
function AppHeader() {
  ...
  return (
    <>
      <nav className={`fixed top-0 left-0 right-0 z-50 w-full bg-white dark:bg-[#0F172A] text-gray-900 dark:text-[#FBFCFE] border-b border-gray-200 dark:border-gray-800 transition-transform duration-800 ${showHeader ? "translate-y-0" : "-translate-y-full"} `} >
          ...
          <div className="hidden md:block md:flex-1 md:max-w-150">
            ...
            (
              <div className="flex items-center justify-center">
                <span className="text-[11px] sm:text-xs md:text-sm text-gray-700 dark:text-gray-400 text-center font-semibold leading-tight">
                  Please signUp OR logIn to explore Student Hub.
                </span>
              </div>
            )}
          </div>
              ...
              <div className="flex items-center gap-2 relative">
                ...
                <div className="hidden md:block relative w-9 h-9 sm:w-10 sm:h-10">
                  <Link href={`/profile/profile`}>
                    <Image
                      src={ userProfileData?.data?.userProfile?.profileImgUrl || "/img/defaultProfile.jfif" }
                      alt="Profile"
                      fill
